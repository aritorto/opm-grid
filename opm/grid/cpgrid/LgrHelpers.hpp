...
std::pair<std::set<Coordinate,FieldVectorLess>,
          std::map<int,std::pair<int,std::vector<Coordinate>>>>
                   ...
                   int& newFaceIdx)
{
    ...
    std::map<int,std::pair<int,std::vector<Coordinate>>> overlapFaces{}; 
    ...
    for (const auto& face : parentCellToFace) {
        ...
        for (const auto& refinedFace : refinedCellToFace) {
            ...
            if (newFace.has_value() && !newFace.value().empty()) {
                ...
                overlapFaces[face.index()] = std::make_pair( overlapFaceIdx, newFace.value());
    ...
    return std::make_pair<std::set<Coordinate,FieldVectorLess>,
                          std::map<int,std::pair<int,std::vector<Coordinate>>>>(std::move(missingVertices),
                                                                                std::move(overlapFaces));
}
