                                   ...
                                   const std::vector<std::array<int,3>>& endIJK_vec)
{
    ...
    all_patch_corners.reserve(this->data_[0] -> size(3)); // reserve more than needed!
    std::vector<int> all_patch_faces;
    all_patch_faces.reserve(this->data_[0]-> face_to_cell_.size()); // reserve more than needed!
    std::vector<int> all_patch_cells;
    all_patch_cells.reserve(this->data_[0]-> size(0)); // reserve more than needed!
    for (int patch = 0; patch < num_patches; ++patch){
        for (const auto& corner : (*(this->data_[0])).getPatchCorners(startIJK_vec[patch], endIJK_vec[patch])) {
            all_patch_corners.push_back(corner);
        }
        for (const auto& face : (*(this->data_[0])).getPatchFaces(startIJK_vec[patch], endIJK_vec[patch])) {
            all_patch_faces.push_back(face);
        }
        for (const auto& cell : (*(this->data_[0])).getPatchCells(startIJK_vec[patch], endIJK_vec[patch])) {
