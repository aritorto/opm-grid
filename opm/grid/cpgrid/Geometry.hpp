...
namespace Dune
{
    namespace cpgrid
    {
        ...
        public:
            ...
            // @todo WIP Refine a path of cells 
            // Refine a patch of cells with 'uniform' regular intervals.
            // (meaning that 'cells_per_dim' argument is the same for each
            // cell of the patch).
            ...
            // @param 8corners_indices_storage       To store the indices of the 8 corners of each child cell of all
            //                                       each parent cells. 
            // void refine_patch(const std::array<int,3>& cells_per_dim,
            //                 std::vector<cpgrid::Geometry<3,3>> patch_to_refine,
            //                   DefaultGeometryPolicy& all_geom,
            //                  std::vector<std::array<int, 8>>&  8corners_indices_storage)
            //{
                // Check the size of "patch_to_refine"
            //      int n = patch_to_refine.size();
                // Get the index of each cell from the patch to be refined
                // [not sure we can actually do this..]
            //      std::vector<int>& cell_to_refine_indices;
            //     for (int l = 0; l < n; ++l) {
            //         cell_to_refine_indice.pusch_back(/*getIndexCell(patch_to_refine[l])*/);
            //     }
                // Sort the vector of indices of the cells from the patch.
            //    cell_to_refine_indices = sort(cell_to_refine_indices.begin(), cell_to_refine_indice.end());
                // Now we have certain order for refine cell by cell...
                // To be continued...
