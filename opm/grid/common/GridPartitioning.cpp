...
void addOverlapLayer(const CpGrid& grid,
                        ...
                        int level)
    {
        ...
        //   const CpGrid::LeafIndexSet& ix = grid.leafIndexSet(); // here need level
        const auto& ix = (level==-1)? grid.leafIndexSet() : grid.levelIndexSet(level);
   ...
   std::tuple<std::vector<int>, std::vector<std::pair<std::string,bool>>,
                         ...
                         int level)
    {
        ...
        if (grid.comm().rank() == 0)
        {
            ...
            auto cell = (level==-1)?  grid.leafbegin<0>() : grid.lbegin<0>(0);
            auto cellEnd = (level==-1)? grid.leafend<0>() : grid.lend<0>(0);
